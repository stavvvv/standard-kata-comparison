...
# Test rates for small load (added 12 as requested)
RATES=(2 4 6 8 10 12)
...
for rate in "${RATES[@]}"; do
    ...
    run_test "Standard Container" "container_targets.txt" $rate "c_small_${rate}.txt"
    
    # Short cooldown after Standard test
    sleep 120
    
    # Test Kata Container  
    run_test "Kata Container" "kata_targets.txt" $rate "k_small_${rate}.txt"
...
for rate in "${RATES[@]}"; do
    echo "  c_small_${rate}.txt - Standard container at ${rate} req/s"
    echo "  k_small_${rate}.txt - Kata container at ${rate} req/s"
done
